...
class SimTrackActor(BaseActor):
    ...
    def forward_pass(self, data, run_box_head, run_cls_head):
        ...
        template_img_i = data['template_images'][0].view(-1, *data['template_images'].shape[2:])  # (batch, 3, 128, 128)
        template_att_i = data['template_att'][0].view(-1, *data['template_att'].shape[2:])  # (batch, 128, 128)
        ...
        feat_dict_list.append(self.net([template_img_i, search_img, data['template_anno'][0].cuda()], mode='backbone'))
